...
export default class Closest extends CommandInterface {
    ...
    async execute(message: Message, data: string[]): Promise<any> {
        // find system
        let systemSearchName = data[0];
        if (systemSearchName.length < 3) {
            await message.channel.send('Region name must be at least 3 characters long.');
            return;
        }
        let searchResults = (await this.esi.get('/v2/search/?categories=solar_system&datasource=tranquility&language=en&strict=false&search=' + systemSearchName)).data.solar_system;
        ...
        let solarSystemId = searchResults[0];
